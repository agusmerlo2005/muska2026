    ...
    <div className="fixed inset-0 z-[200]">
      ...
      <div className="absolute inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        <div className="p-6 border-b border-gray-50 flex justify-between items-center">
          ...
          <button onClick={onClose} className="p-2 hover:bg-gray-50 rounded-full transition-colors"><X size={18} /></button>
        </div>
        ...
        {cart.length > 0 && (
          <div className="p-6 border-t border-gray-50">
            ...
            <Link href="/checkout" onClick={onClose} className="w-full block text-center bg-black text-white py-5 text-[10px] uppercase font-black tracking-[0.3em]">Finalizar Compra</Link>
          </div>
        )}
      </div>
    </div>
