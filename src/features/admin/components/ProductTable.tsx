      ...
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {initialProducts.map((prod) => (
          <div key={prod.id} className="border border-gray-100 p-4 flex gap-4 items-center bg-white">
            <img src={prod.image_url} className={`w-16 h-16 object-cover shrink-0 ${prod.stock <= 0 ? 'grayscale opacity-30' : 'grayscale'}`} />
            <div className="flex-1 min-w-0">
              ...
              <div className="flex gap-2 mt-1">
                ...
                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-sm ${prod.stock <= 0 ? 'bg-red-600 text-white' : prod.stock < 5 ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-400'}`}>
          ...
          <tbody className="divide-y divide-gray-50">
            {initialProducts.map((prod) => (
              <tr key={prod.id} className="hover:bg-gray-50/50 transition-colors group">
                <td className="p-4 w-16"><img src={prod.image_url} className={`w-10 h-10 object-cover ${prod.stock <= 0 ? 'grayscale opacity-30' : 'grayscale'}`} /></td>
                <td className="p-4">
                ...
                <td className="p-4 text-[9px] font-black tracking-widest">
                  <span className={prod.stock <= 0 ? 'text-red-600 bg-red-50 px-2 py-1' : prod.stock < 5 ? 'text-orange-500' : 'text-gray-400'}>
